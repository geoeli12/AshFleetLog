...
export default function BrokenTrailerPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await api.entities.BrokenTrailer.list("-date");
  ...
  const createMutation = useMutation({
    mutationFn: (data) => api.entities.BrokenTrailer.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => api.entities.BrokenTrailer.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["brokenTrailers"] });
    }
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.entities.BrokenTrailer.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["brokenTrailers"] });
    }
  });
